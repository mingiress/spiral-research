...
if [ "$STATUS" = "pending" ]; then
    ...
    echo "Research initiated (request_id: $REQUEST_ID), polling for results..."
    ...
    while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
        ...
        POLL_STATUS=$(echo "$POLL_RESPONSE" | jq -r '.status // "error"')
        
        if [ "$POLL_STATUS" = "completed" ]; then
            echo "Research completed!"
            RESPONSE="$POLL_RESPONSE"
            break
        elif [ "$POLL_STATUS" = "error" ] || [ "$POLL_STATUS" = "failed" ]; then
            echo "Error: Research failed"
            echo "$POLL_RESPONSE"
            exit 1
        fi
        
        echo "Still researching... (attempt $ATTEMPT/$MAX_ATTEMPTS)"
    done
    
    if [ $ATTEMPT -ge $MAX_ATTEMPTS ]; then
        echo "Error: Research timed out after 180 seconds"
        echo "Request ID: $REQUEST_ID"
        echo "You can manually check: curl -H \"Authorization: Bearer \$TAVILY_API_KEY\" https://api.tavily.com/research/$REQUEST_ID"
        exit 1
    fi
fi
